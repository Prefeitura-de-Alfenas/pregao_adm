...
import { GetModalidadeById, UpdateModalidade } from "@/app/api/modalidade/routes";
...
interface EditModalidadeProps{
    IdModalidade:string,
    usuario:UsuarioLogadoI
}

function EditarModalidade({usuario,IdModalidade}:EditModalidadeProps ) {
  ...
  const { handleSubmit,register,setValue,formState:{errors}} = useForm<FormData>({
    ...
    const {data,isLoading } = useQuery({
      queryKey:['modalidade',IdModalidade],
      queryFn:() => GetModalidadeById(usuario,IdModalidade as string),
  ...
  const mutation = useMutation({
    mutationFn: (data:FormData) => {
      return     UpdateModalidade(usuario,IdModalidade,data)
