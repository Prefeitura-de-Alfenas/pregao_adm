...
import { GetModalidades } from "@/app/api/modalidade/routes";

interface TableBeneficiosProps{
  usuario:UsuarioLogadoI
}
const TableModadlidade = ({usuario}:TableBeneficiosProps) => {
  ...
  const {data,isPending,isError,error} = useQuery({
    queryKey:['modalidades',skip,search],
    queryFn:() => GetModalidades(usuario,skip,search),
 ...
 const handelClickSearcher = () =>{
  ...
  if (isError) {
    ...
    return ( 
        ...
        <Button className="ms-1 mt-4 mb-4 text-white font-bold"><Link href="/modalidade/create">Nova Modalidade</Link></Button>
        </div> 
        ...
        <TableBody>
            ...
            {data?.map((modalidade:MoadalidadeI) => (
             <TableRow key={modalidade.IdModalidade}>
                <TableCell className="font-medium">{modalidade.Nome}</TableCell>
              

                <TableCell><Link href={`/modalidade/edit/${modalidade.IdModalidade}`} ><FileEdit fill="#312e81" /></Link></TableCell>
...
export default TableModadlidade