...
import { CreateContrato, GetProcessoContratoOrgao } from "@/app/api/contrato/routes";
import { ProcessoxOrgaoI } from "@/interfaces/Orgao/inteface";
import { ProcessoxFornecedorI } from "@/interfaces/Processo/inteface";
import { ProcessoContratoxFornecedorI } from "@/interfaces/fornecedor/inteface";
...
interface CriarContratoProps{
  IdProcesso:string,
  usuario:UsuarioLogadoI
}

function CriarContrato({usuario,IdProcesso}:CriarContratoProps) {
  ...
  const { handleSubmit,register,setValue,getValues,formState:{errors}} = useForm<FormData>({
   ...
   const {data: dataAllProcesso,isLoading:isLoadingSitacao } = useQuery({
    queryKey:['processoall',IdProcesso],
    queryFn:() => GetProcessoContratoOrgao(usuario,IdProcesso),
  ...
  const mutation = useMutation({
    mutationFn: async (data:FormData) => {
      ...
      return     CreateContrato(usuario,dataResponse)
   ...
   useEffect(() =>{
    setValue("IdModalidade", dataAllProcesso ? dataAllProcesso.IdModalidade :'',{ shouldValidate: true });
    setValue("Numeroprocesso", dataAllProcesso ? dataAllProcesso.Numero :'',{ shouldValidate: true });
   },[dataAllProcesso,setValue])
    ...
    return (  
      ...
      <div className=" mx-auto p-4 gap-4">
        {/* Coluna 1 */}
         ...
         <div className="mb-4">
            <label htmlFor="IdFornecedor" className="block text-sm font-medium text-white">Sitação:</label>
            <select id="IdFornecedor"   {...register('IdFornecedor')} required className="mt-1 p-2 w-full border rounded-md mb-2 bg-background">
               {dataAllProcesso.ProcessoXFornecedor.map((item:ProcessoContratoxFornecedorI) =>(
                   <option key={item.IdFornecedor} value={item.IdFornecedor}>{item.fornecedor.Empresa}</option>
               ))}
            </select>
          ...
          <div className="mb-4">
            ...
            <select id="IdOrgao"   {...register('IdOrgao')} required className="mt-1 p-2 w-full border rounded-md mb-2 bg-background">
               {dataAllProcesso.ProcessoxOrgao.map((item:ProcessoxOrgaoI) =>(
                   <option key={item.IdOrgao} value={item.IdOrgao}>{item.orgao.Nome}</option>
               ))}
            </select>
            ...
            <div className="mb-4 flex-1">
              <label htmlFor="IdModalidade" className="block text-sm font-medium text-white">Modalidade do Pregão:</label>
              <Input type="text" id="IdModalidade" {...register('IdModalidade')} required readOnly className="mt-1 p-2 w-full border rounded-md mb-2 bg-transparent"  />
              <Input type="text" id="NomeModalidade"  required readOnly className="mt-1 p-2 w-full border rounded-md mb-2 bg-transparent" value={dataAllProcesso.Modalidade.Nome} />
