  ...
  if (isError) {
    ...
    return ( 
        ...
        <TableBody>
            ...
            {data?.map((processo:ContratoI) => (
             <TableRow key={processo.IdContratoAditivos}>
                <TableCell className="font-medium">{processo.NumeroContrato}</TableCell>
                <TableCell className="font-medium">{processo.Numeroprocesso}</TableCell>
                <TableCell className="font-medium">{processo.NumeroPregao}</TableCell>
                <TableCell className="font-medium">{processo.Modalidade.Nome}</TableCell>
                <TableCell className="font-medium">{convertDataParaPtBr(processo.DataPublicacao)}</TableCell>
            
              
                <TableCell><Link href={`/arquivo/${processo.IdContratoAditivos}`} ><FileBox   fill="#312e81" /></Link></TableCell>
                <TableCell><Link href={`/processo/processoes/${processo.IdContratoAditivos}`} ><Box  fill="#312e81" /></Link></TableCell>
                <TableCell><Link href={`/processo/edit/${processo.IdContratoAditivos}`} ><FileEdit  fill="#312e81" /></Link></TableCell>

                <TableCell>
                    <DeleteSoftProcesso id={processo.IdContratoAditivos} refetch={refetch} usuario={usuario} />
                </TableCell>
