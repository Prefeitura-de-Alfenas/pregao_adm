...
import { GetModaliadeSituacao, GetProcessoById, UpdateProcesso } from "@/app/api/processo/routes";
...
function EditarProcesso({usuario,IdProcesso}:EditProcessoProps ) {
  ...
  const { handleSubmit,register,setValue,formState:{errors}} = useForm<FormData>({
    ...
    useEffect(() => {
      setCheckPrefeitura(false)
      setCheckSaude(false)
      if(data){  
       if(data.NumeroprocessoPrefeitura){
        setCheckPrefeitura(true)
       }
       if(data.NumeroprocessoSaude){
        setCheckSaude(true)
       }
      }
      setValue("IdSituacao", data ? data.IdSituacao :'',{ shouldValidate: true });
      setValue("IdModalidade", data ? data.IdModalidade :'',{ shouldValidate: true });
      setValue("NumeroprocessoPrefeitura", data ? data.NumeroprocessoPrefeitura :'',{ shouldValidate: true });
      setValue("NumeroprocessoSaude", data ? data.NumeroprocessoSaude :'',{ shouldValidate: true });
      setValue("Numero", data ? data.Numero :'',{ shouldValidate: true });
      setValue("ValorLicitacao", data ? data.ValorLicitacao :'',{ shouldValidate: true });
      setValue("DataAbertura", data ? converterDataParaFormatoInputDate(data.DataAbertura) :'',{ shouldValidate: true });
  ...
  const mutation = useMutation({
    mutationFn: (data:FormData) => {
      return     UpdateProcesso(usuario,IdProcesso,data)
      ...
      { !isLoading ?
        ...
        <div className="flex items-center  mb-6 mt-6 gap-14">
           <div className="flex items-center space-x-2">
             <input type="checkbox" id="checkPrefeitura" onChange={handleSetCheckPrefeitura} checked={checkPrefeitura}   />
           ...
           <div className="flex items-center space-x-2"> 
            <input type="checkbox" id="checkSaude" onChange={handleSetCheckSaude} checked={checkSaude} />
        ...
        <Button className="text-white font-bold mt-2" disabled={validationFundo()}>Atualizar</Button>
